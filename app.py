...
from pending_delay.schema import TARGET_COL, LEAKED_COLUMNS, DROP_METADATA, CATEGORICAL_FEATURES
...
tab_results, tab_review, tab_data, tab_analytics, tab_features = st.tabs([
    "Model Results", "HIGHER Review", "Data Explorer", "Analytics", "Feature Analysis",
])
...
with tab_results:
    ...
    if ope:
        ...
        tier_data = pd.DataFrame({
            ...
            "Count": [ope["skip_count"], ope["lower_count"], ope["static_count"], ope["higher_count"]],
        })
        tier_data["Pct"] = tier_data["Count"] / tier_data["Count"].sum() * 100
        fig_tier = px.bar(
            tier_data, x="Tier", y="Count", color="Tier",
            color_discrete_map={"SKIP": "#2ecc71", "LOWER": "#3498db", "STATIC": "#f39c12", "HIGHER": "#e74c3c"},
    ...
    if not review_path.exists():
        ...
        with col_a:
            ...
            fig_scatter = px.scatter(
                df_rev, x="predicted_clv", y="odds_after_10", color="sharp",
                color_discrete_map={"Sharp (< -0.01)": "crimson", "Not sharp": "steelblue"},
                hover_data=["sport", "market_name", "stake", "ticket_state"],
                opacity=0.7,
            )
            fig_scatter.add_shape(type="line", x0=-0.3, x1=0.3, y0=-0.3, y1=0.3,
        ...
        with col_b:
            ...
            fig_hist = px.histogram(
                df_rev, x="odds_after_10", color="sharp", nbins=25,
                color_discrete_map={"Sharp (< -0.01)": "crimson", "Not sharp": "steelblue"},
        ...
        with col_d:
            ...
            fig_pie = px.pie(
                ...
                color="state",
                color_discrete_map={"rejected": "#e74c3c", "won": "#2ecc71",
                                    "lost": "#3498db", "accepted": "#95a5a6"},
            )
        ...
        with col_e:
            ...
            fig_stake = px.scatter(
                df_rev, x="stake", y="odds_after_10", color="predicted_clv",
        ...
        with col_f:
            ...
            fig_hist_clv = px.scatter(
                df_rev, x="bs_avg_odds_after_10", y="odds_after_10", color="sharp",
                color_discrete_map={"Sharp (< -0.01)": "crimson", "Not sharp": "steelblue"},
        ...
        show_cols = [
            "sport", "market_name", "selection_odds", "stake", "pending_delay",
            "predicted_clv", "odds_after_10", "odds_after_30", "ticket_state",
            "bs_avg_odds_after_10", "bettor_id",
        ]
...
with tab_analytics:
    ...
    with col_a:
        ...
        """).df()
        fig_states = px.bar(state_df, x="ticket_state", y="n", text="pct",
                            color="ticket_state")
    ...
    with col_b:
        ...
        """).df()
        if len(rej_df) > 0:
            fig_rej = px.bar(rej_df, x="reject_reason", y="n", text="pct",
                             color="reject_reason")
...
with tab_features:
    ...
    sample_size = st.sidebar.number_input("Sample size", value=200_000,
                                          min_value=10_000, max_value=2_000_000, step=50_000)
    fa_rounds = st.sidebar.number_input("LightGBM rounds", value=200,
    ...
    if st.sidebar.button("Run Feature Analysis", type="primary"):
        ...
        dtrain = lgb.Dataset(features, label=target,
                             categorical_feature=cat_indices, free_raw_data=False)

        with st.spinner("Training quick LightGBM..."):
            booster = lgb.train(
                {"objective": "huber", "metric": "mae", "num_leaves": 63,
                 "learning_rate": 0.05, "feature_fraction": 0.8, "bagging_fraction": 0.8,
                 "bagging_freq": 5, "min_child_samples": 100, "verbose": -1},
                dtrain, num_boost_round=fa_rounds,
            )

        gain = dict(zip(features.columns, booster.feature_importance(importance_type="gain").tolist()))
        split = dict(zip(features.columns, booster.feature_importance(importance_type="split").tolist()))

        imp_df = pd.DataFrame({
            "feature": list(gain.keys()), "gain": list(gain.values()),
        ...
        st.subheader("Feature Importance (Gain)")
        fig_gain = px.bar(imp_df.head(25), x="gain_pct", y="feature", orientation="h",
                          height=500, labels={"gain_pct": "Gain %"})
        ...
        if numeric_cols:
            ...
            fig_corr = px.bar(corr_df.head(25), x="abs_correlation", y="feature",
        ...
        if run_perm:
            ...
            fig_perm = px.bar(perm_df.head(25), x="mae_increase", y="feature",
                              orientation="h", height=500)
        ...
        selected = st.multiselect("Features to use", options=imp_df["feature"].tolist(),
                                  default=default_sel)
        if selected:
